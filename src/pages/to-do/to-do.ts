 ...
 let params = 'email='+this.myJData.email;
   ...
   get_MessageList()
  {
      ...
      let c : String="";
              ...
              this.http.post(this.myJData.url+'getAllToDoMessages.php',body,{headers:headers})
              .map(res => res.json() )
              .subscribe(data=> {
              ...
              for(let i=0; i<this.messageList.length;i++)
                {
                  ...
                  if(this.messageList[i].folder.toLowerCase().indexOf('done')>0)
                    {
                      this.iconArray[i]='done-all';
                    }
                  else if(this.messageList[i].folder.toLowerCase().indexOf('tiny')>0)
                    {
                      this.iconArray[i]='clipboard';
                    }
                  else if(this.messageList[i].folder.toLowerCase().indexOf('schedule')>0)
                    {
                      this.iconArray[i]='flag';
                    }
